...
import { getWorkEmailLabel, getSignUpErrorMessage, getWrongOTPErrorMessage,getSignUpButton } from "./user_interface/trailBlazzerModalSelectors";
...
export async function verifyTheSignUpError(page){
  let getSignUpErrorMessageText
  try{
    await page.waitForSelector(getSignUpErrorMessage);
    let text = await page.$(getSignUpErrorMessage);
    getSignUpErrorMessageText = await text.evaluate((status) => status?.innerText);
  }catch{
